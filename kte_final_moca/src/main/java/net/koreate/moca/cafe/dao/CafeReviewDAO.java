...
public interface CafeReviewDAO {
	...
	@Select("SELECT month(time) AS criteria,avg(star) AS score FROM tbl_cafe_review WHERE cafe_no = #{cafe_no} AND time between now() - interval 5 month and now() GROUP BY month(time) ORDER BY criteria ASC")
	List<ReviewCounterVO> monthCount(int cafe_no);
	
	@Select("SELECT star AS criteria,count(*) AS score FROM tbl_cafe_review WHERE cafe_no = #{cafe_no} GROUP BY star ORDER BY criteria DESC")
	List<ReviewCounterVO> starCount(int cafe_no);
	
	@Select("SELECT avg(star) FROM tbl_cafe_review WHERE cafe_no = #{cafe_no}")
